...
import { Target, Plus, Brain, Users, Calendar, TrendingUp } from 'lucide-react';
...
const GoalsPage: React.FC = () => {
  ...
  const [goalMatches, setGoalMatches] = useState<GoalMatch[]>([]);
  ...
  const loadGoalMatches = (goalId: string) => {
    fetch(`/api/goals/${goalId}/matches`)
      .then(res => res.json())
      .then(data => setGoalMatches(data.matches || []))
      .catch(err => console.error('Failed to load goal matches:', err));
  };

  const handleGoalSelect = (goal: Goal) => {
    setSelectedGoal(goal);
    loadGoalMatches(goal.id);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-gray-800">
      <div className="container mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button 
            className="glass-button px-6 py-3 rounded-lg text-blue-400 border border-blue-400/30 hover:bg-blue-400/10 flex items-center"
